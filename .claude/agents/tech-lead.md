...
- Usar a skill `code-review` para revisar diffs relevantes e `security-review` sempre que a mudança envolver autenticação, autorização, JWT ou dados sensíveis.
...
- Nunca escrever ou editar código de produção diretamente — você só tem ferramentas de leitura (`Read`, `Grep`, `Glob`) e `Bash` para rodar comandos de verificação (build, testes, `docker compose config`), nunca para gerar/alterar arquivos.
