...
Use o agente especializado adequado para cada tarefa, conforme a área de responsabilidade descrita:

| Agente | Responsabilidade |
|--------|------------------|
| `tech-lead` | Decisões de arquitetura que cruzam múltiplos módulos, arbitragem das "Decisões em aberto" do SPEC, revisão (sem implementar) de mudanças estruturais. |
| `explore` | Exploração rápida do código: localizar arquivos, procurar padrões, responder perguntas sobre a estrutura do projeto. |
| `qa-engineer` | Execução e report de builds, testes, cobertura e critérios de aceitação. Não corrige código — apenas reporta falhas. |
| `db-migration-engineer` | Migração do banco de dados de PostgreSQL para MySQL (driver TypeORM e Docker Compose). |
| `backend-domain-engineer` | Modelagem de domínio no backend (entidades TypeORM, enums, interfaces e repositórios). |
| `backend-auth-engineer` | Autenticação e autorização no backend (JWT, login, middlewares, guardas). Código sensível — revisão de segurança obrigatória. |
| `backend-comentarios-engineer` | Casos de uso, controllers e rotas de Comentários. |
| `frontend-setup-engineer` | Bootstrap inicial do projeto frontend (Vite + React + TS + Tailwind + Vitest). |
| `frontend-feature-engineer` | Features do frontend que consomem a API (auth, posts, comentários, área administrativa). |
| `frontend-ui-engineer` | Layout compartilhado e responsivo, navegação condicional, página 404, revisão do roteador. |
| `general` | Pesquisas e tarefas multi-etapa que não se encaixam em um agente específico. |